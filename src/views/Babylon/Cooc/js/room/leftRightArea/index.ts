import {
  ...
  import {createLeftAreaBot} from './area1'
  export function createDTRgihtArea({ scene }) {
    let wrap = null;
    wrap = BABYLON.MeshBuilder.CreateBox(
      "Conference",
      { height: 6, width: 143, depth: 304 },
      scene
    );
    ...
    let areaBot = createLeftAreaBot({scene})
    areaBot.parent = wrap
    initRepeatPosition({ parent: wrap, child: areaBot });
    areaBot.position =areaBot.position.add(new BABYLON.Vector3(0,0,224))
    return wrap;
  }
